...
run(function()
  ...
  skywars = setmetatable({
    ...
    Roact = require(
      roactCheck and roactCheck.src
        or replicatedStorage['rbxts_include']['node_modules']['@rbxts'].ReactLua['node_modules']['@jsdotlua']['roact-compat']
    ),
  ...
  task.spawn(function()
    repeat
      if entitylib.isAlive then
        entitylib.character.GroundPosition = entitylib.character.Humanoid.FloorMaterial ~= Enum.Material.Air
            and entitylib.character.RootPart.Position
          or entitylib.character.GroundPosition
      end
      task.wait()
      ...
      repeat
        ...
        if held then
          if held.Rewrite and Blocks.Enabled then
            local block = skywars.ItemMeta[held.Rewrite.Type:gsub(
              '{TeamId}',
              skywars.TeamController:getPlayerTeamId(lplr) or 'White'
            )]
        ...
        skywars.SprintingController.disableSprinting = function(tab, ...)
          ...
          if not tab.canSprint then
            task.spawn(function()
              repeat
                task.wait(0.1)
              until tab.canSprint or not Sprint.Enabled
...
run(function()
  ...
  local function velocityFunction(velo, ...)
    ...
    local check = not Targeting.Enabled
    ...
    Function = function(callback)
      if callback then
        connection = getconnections(
          debug.getupvalue(
            debug.getupvalue(skywars.Remotes[remotes['PlayerVelocityController:onStart']].connect, 1).fireClient,
            1
          ).OnClientEvent
        )[1]
                ...
                if Attacking then
                  ...
                  for _, v in anims[AnimationMode.Value] do
                    AnimTween = tweenService:Create(
                      ViewmodelMotor,
                      TweenInfo.new(
                        first and (AnimationTween.Enabled and 0.001 or 0.1) or v.Time / AnimationSpeed.Value,
          ...
          if entitylib.isAlive then
            ...
            if (entitylib.character.GroundPosition.Y - entitylib.character.RootPart.Position.Y) > 10 then
              ...
              local ray = workspace:Raycast(
                ...
                Vector3.new(0, -(entitylib.character.HipHeight + 10), 0),
                rayCheck
              )
              if not ray then
...
run(function()
  ...
  local function getProjectiles()
    ...
    for slot, item in store.inventory do
      ...
      if
        item.Ranged
        and table.find(List.ListEnabled, item.Ranged.ProjectileType)
        and getItem(item.Ranged.ProjectileType)
      then
            ...
            if wool then
              ...
              for i = Expand.Value, 1, -1 do
                  ...
                  root.Position
                    - Vector3.new(
                      ...
                      entitylib.character.HipHeight
                        + (Downwards.Enabled and inputService:IsKeyDown(Enum.KeyCode.LeftShift) and 4.5 or 1.5),
                      0
                    )
                ...
                if not block then
                  ...
                  if blockpos then
                    local block = skywars.ItemMeta[wool.Rewrite.Type:gsub(
                      '{TeamId}',
                      skywars.TeamController:getPlayerTeamId(lplr) or 'White'
                    )]
    ...
    Function = function(callback)
      Functions[2] = callback
          and function(currencytable, shop, upgrades)
            buyUpgrade(
              store.tools.sword and store.tools.sword.Name,
              skywars.Shop.Blacksmith.ItemUpgrades[2],
              currencytable
            )
          end
        or nil
  ...
  Pickaxe = AutoBuy:CreateToggle {
    ...
    Function = function(callback)
      Functions[3] = callback
          and function(currencytable, shop, upgrades)
            buyUpgrade(
              store.tools.pickaxe and store.tools.pickaxe.Name,
              skywars.Shop.Blacksmith.ItemUpgrades[3],
              currencytable
            )
          end
        or nil
          ...
          if entitylib.isAlive and store.hand then
            ...
            for i, v in eggs do
              if v.PrimaryPart and (localPosition - v.PrimaryPart.Position).Magnitude < Range.Value then
                ...
                if v:GetAttribute 'TeamId' == lplr:GetAttribute 'TeamId' then
                  continue
                end
      ...
      if callback then
        ...
        LunarVape:Clean(runService.RenderStepped:Connect(function()
          if ViewmodelTool then
            local dcf = (
              (CFrame.new(2.06, -2.44, -2.24) * CFrame.new(0.6, -0.2, -0.6))
              * CFrame.Angles(math.rad(99), math.rad(2), math.rad(-4))
            ) * ViewmodelMotor.C0
