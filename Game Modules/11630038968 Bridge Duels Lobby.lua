...
run(function()
  ...
  local ParticleSize
  local LegitAura
  local Particles, Boxes, AutoHitReset, AttackDelay, SwingDelay, ClickDelay, DodgeDelay, FocusConnection =
    {}, {}, {}, tick(), tick(), tick(), tick(), nil
  ...
  Killaura = LunarVape.Categories.Blatant:CreateModule {
    ...
    Function = function(callback)
      if callback then
          ...
          if tool and tool:HasTag 'Sword' then
            ...
            if #plrs > 0 then
              ...
              for _, v in next, plrs do
                if bd.Entity.LocalEntity.State.TeamColour == Color3.new(1, 1, 1) and v.RootPart.Position.Y > 0 then
                  continue
                end
                ...
                if AttackDelay < tick() then
                  if LunarVape.Modules.AntiHit.Enabled and not LegitAura.Enabled and DodgeDelay < tick() then
                    ...
                    task.spawn(function()
                      ...
                      if not FocusConnection then
                        FocusConnection = runService.PreSimulation:Connect(function()
                          if not (vhrp and hrp) then
                            print 'bye'
                            FocusConnection:Disconnect()
                            return
                          end

                          local pos = hrp.Position - vhrp.Position
                          pos = Vector3.new(pos.X, 0, pos.Z)

                          if pos.Magnitude >= SwingRange.Value then
                            print 'bye'
                            FocusConnection:Disconnect()
                            return
                          end
                          ...
                          print 'hi'
                          gameCamera.Viewmodel.Head.CFrame =
                            CFrame.new(Vector3.new(hrp.Position.X, vhrp.Position.Y, hrp.Position.Z))
                        end)
                      end
                  ...
                  if bdent then
                    bd.Blink.item_action.attack_entity.fire {
                      ...
                      is_crit = LunarVape.Modules.Criticals and LunarVape.Modules.Criticals.Enabled and true
                        or entitylib.character.RootPart.AssemblyLinearVelocity.Y < 0,
        ...
        if LunarVape.Modules.AntiHit and LunarVape.Modules.AntiHit.Enabled then
          LunarVape.Modules.AntiHit:Toggle()
        end
        for _, v in next, AutoHitReset do
          if v then
            v:Disconnect()
          end
        end
            ...
            if pickaxe then
              ...
              for blockpos, block in getBlocksInPoints(pos - rvec, pos + rvec) do
                if
                  ...
                  and (block.Parent.Name == 'Bed' and lplr.Team and block.Parent:GetAttribute 'Team' ~= lplr.Team.Name)
                then
                  breakBlock = block
                  break
                end
