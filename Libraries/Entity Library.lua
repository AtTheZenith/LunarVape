  ...
  if entitylib.isAlive then
    ...
    for _, v in sortingTable do
      if entitysettings.Wallcheck then
        if
          entitylib.Wallcheck(entitysettings.Origin, v.Entity[entitysettings.Part].Position, entitysettings.Wallcheck)
        then
          continue
        end
      end
...
entitylib.AllPosition = function(entitysettings)
  ...
  if entitylib.isAlive then
    ...
    for _, v in entitylib.List do
      ...
      local mag = not entitysettings.IgnoreY and (v[entitysettings.Part].Position - localPosition).Magnitude
        or (Vector3.new(v[entitysettings.Part].Position.X, 0, v[entitysettings.Part].Position.Z) - Vector3.new(
          localPosition.X,
          0,
          localPosition.Z
        )).Magnitude
